...
namespace TimeElapse
{
    public class Logic
    {
        ...
        public TimeSpan CalculateReaminTimeInSeconds(int year, int month, int day)
        {
            var now = DateTime.Now;
            var bye = new DateTime(year, month,day, 12, 00, 00);
            var result = bye - now;
            return result;
        }
        ...
        public (long, double) PorcentageRemain(int year, int month, int day)
        {
            var dateStart = new DateTime(2019, 02, 21, 12, 0, 0);
            var dateEnd = new DateTime(year, month, day, 12, 0, 0);
            var currentDate = DateTime.Now;
